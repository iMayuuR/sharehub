...
- **The Transfer Sheet Sat On Top of Everything** — It listened for touch only, so on a desktop there was no way to move it at all, and it covered the peer cards with no way out. It now collapses to a bar by tapping or dragging its grip, closes with a button or a downward drag, gets out of the way by itself once every transfer has finished, and its list scrolls inside a fixed frame instead of running off the screen.
- **A Batch of Files Buried the Screen in Toasts** — Every file raised one on arrival and a second saying the same thing, so four files meant eight overlapping notifications over the whole app. Duplicates now refresh the existing toast, the stack is capped, and per-file chatter is left to the sheet, which is what it is for.
- **Peers Showed as "Unknown Device"** — A device that had joined but not yet announced itself, or whose announcement never arrived, sat there unnamed. It now gets a friendly name derived from its peer id, so it is stable and identical on both screens until the real one lands.
